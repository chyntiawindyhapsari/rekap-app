...
@section('content')
  ...
  <div class="p-4" style="flex-grow: 1;">
    <div class="container">
        <h2 class="text-center text-dark mb-4"><b>Edit Data Pembelian Produk</b></h2>
        ...
        <form action="{{ route('produk.update', $data->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="card shadow-sm p-4 mb-4">
                ...
                <div class="form-group">
                    ...
                    <textarea name="alamat_toko" id="alamat_toko" class="form-control @error('alamat_toko') is-invalid @enderror" rows="4" placeholder="Masukkan alamat toko di sini" required>{{ old('alamat_toko', $data->alamat_toko) }}</textarea>
                    @error('alamat_toko')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
                <div class="form-group">
                    ...
                    <input type="file" name="desain" id="desain" class="form-control @error('desain') is-invalid @enderror" accept=".png, .jpg, .jpeg">
                    <small class="form-text text-muted">Biarkan kosong jika tidak mengubah desain</small>
                    @error('desain')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                    @if ($data->desain)
                        <div class="mt-2">
                            <label>Desain Saat Ini:</label><br>
                            <img src="{{ asset('images/' . $data->desain) }}" alt="Desain" class="img-thumbnail" style="max-width: 200px;">
                        </div>
                    @endif
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="bentuk_desain" id="bentuk_desain" class="form-control @error('bentuk_desain') is-invalid @enderror" value="{{ old('bentuk_desain', $data->bentuk_desain) }}" required>
                    @error('bentuk_desain')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="ukuran" id="ukuran" class="form-control @error('ukuran') is-invalid @enderror" value="{{ old('ukuran', $data->ukuran) }}" required>
                    @error('ukuran')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="jumlah_pesanan" id="jumlah_pesanan" class="form-control @error('jumlah_pesanan') is-invalid @enderror" value="{{ old('jumlah_pesanan', $data->jumlah_pesanan) }}" required>
